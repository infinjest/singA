...
- **`reload` мог не перезапускать sing-box** — `procd` сравнивал только параметры инстанса (командная строка не меняется от reload к reload) и решал, что перезапуск не нужен, оставляя старый процесс со старым конфигом в памяти даже после `singbox-compiler` переписывал JSON на диске. Из-за этого новые `custom_rule`/`dns_rule` (исключения маршрутизации, кастомный DNS для домена) применялись только после ручного `restart`. Добавлен `procd_set_param file "$RUN_CONFIG"` — procd теперь отслеживает сам файл.
- **Самоподдерживающаяся цепочка reload'ов** — `start_service()` при "тёплом" кэше подписок форкала фоновую задачу `(sub-updater; reload)`, но `reload_service()` сама вызывает `start_service()`, поэтому каждый reload порождал новый такой job, который порождал следующий, и так по кругу без остановки. Ограничено флагом `/var/run/singbox-bg-synced` — синхронизация теперь выполняется один раз за жизнь сервиса.

### Added

- **Кнопка «Включить/выключить все узлы»** на вкладке «Узлы и Подписки».

### Changed

- **Режим 1** («всё в proxy»): блок «Обновление баз» скрыт в UI (базы geosite/geoip в этом режиме не используются), уведомление при переходе в режим 1 — «Применено. Режим маршрутизации изменён.» (без упоминания баз).
- **Таблица узлов**: колонка «Тип» → «Протокол» (`amneziawg` → `amneziaWG2`, `vless` → `vless-<transport>-<security>`, например `vless-xhttp-reality`); 2 и 3 октет IPv4-адреса сервера маскируются звёздочками; длинный тег (из подписки) обрезается до хвоста после последнего `#`/`@` — не ломает вёрстку шапки.
- **Кнопка «Поделиться»**: убрана попытка записи в буфер обмена и связанное с ней уведомление — `navigator.clipboard` требует HTTPS, а UI отдаётся по HTTP, так что копирование фактически никогда не срабатывало, при этом UI утверждал обратное. Теперь только скачивание файла.

### Removed

- Мёртвый код колонки Latency (`latencyBadge`, `refreshLatencyInTable`, `latencyMap`) — значение всегда было заглушкой «—».
