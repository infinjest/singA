...
Прозрачный прокси-шлюз для OpenWrt. Перехватывает трафик через TProxy и направляет заблокированные ресурсы в туннель — без настройки на клиентских устройствах. Веб-интерфейс на порту `1104`, без LuCI
...
**Маршрутизация**
- 4 режима (`route_mode`): 1) всё в proxy; 2) всё в proxy, кроме РФ-доменов; 3) обход блокировок РКН (дефолт); 4) всё в direct
...
sh /usr/sbin/singbox-uninstall        # полное удаление (--purge для удаления UCI-конфига)
```
