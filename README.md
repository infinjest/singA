...
`/usr/sbin/singbox-logtail` — читается `logread -f -e sing-box` в отдельном, независимом от самого `sing-box` procd-инстансе (`sing-box-logtail`): кольцевой буфер последних 100 строк в `/var/run/sing-box_log.txt` (tmpfs) для лог-вьюера в вебке, вместо `logread -e sing-box | tail -50` на каждый запрос (тот мешает вывод sing-box с другими сервисами, помеченными похоже). Раньше сидел прямо в stdout самого `sing-box` (`sh -c "sing-box ... | singbox-logtail"`) — из-за этого `procd` трекал PID шелл-обёртки, а не `sing-box`, и при потере сигнала оба процесса могли пережить `stop`/`restart` осиротевшими, продолжая маршрутизировать трафик в обход выключенного через UI прокси. Вынесен в отдельный инстанс намеренно: даже если тот же риск когда-нибудь повторится здесь, страдает только свежесть лога, а не работоспособность самого прокси (см. `initd--sing-box.sh`, `CHANGELOG` 0.10.5). В отличие от файлов выше, не хранится отдельным файлом в `src/` — генерируется через `cat << EOF` прямо внутри `install.sh`, тем же способом, что и RPCD ACL и UCI-дефолты (см. `install.sh`, шаг «[4/8]»).
